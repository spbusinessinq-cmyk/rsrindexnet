...
export default function LeftPanel({ hoveredSegment, segments, platform }: LeftPanelProps) {
  ...
  return (
    <div className="hidden lg:flex flex-col shrink-0"
      style={{ borderRight: "1px solid rgba(245,158,11,0.08)", width: 276, background: "rgba(0,0,0,0.12)" }}>

      {/* ── Identity block ─────────────────────────────────────── */}
      <div className="px-6 pt-5 pb-5" style={{ borderBottom: "1px solid rgba(245,158,11,0.08)" }}>
        <div className="font-mono-tactical tracking-widest uppercase mb-1"
          style={{ color: "rgba(245,158,11,0.38)", letterSpacing: "0.22em", fontSize: "8px" }}>
          RSR Intelligence Network
        </div>
        <h2 className="font-orbitron font-bold tracking-wide leading-none mb-1"
          style={{ color: "#F59E0B", textShadow: "0 0 28px rgba(245,158,11,0.18)", letterSpacing: "0.04em", fontSize: "36px" }}>
          INDEX
        </h2>
        ...
        {activeSegment ? (
          <div key={activeSegment.label} className="panel-fade-in space-y-4">
            <div className="font-mono-tactical tracking-widest uppercase"
              style={{ color: "rgba(245,158,11,0.45)", fontSize: "8.5px", letterSpacing: "0.22em" }}>
              Sector Preview
            </div>
              ...
              {activeSegment.detail && (
                <p className="font-mono-tactical leading-relaxed"
                  style={{ color: "rgba(185,205,200,0.62)", lineHeight: "1.88", fontSize: "10px",
                    borderTop: "1px solid rgba(245,158,11,0.07)", paddingTop: 10, marginTop: 10 }}>
                  {activeSegment.detail}
                </p>
              )}
              {activeSegment.methodology && (
                <div className="font-mono-tactical italic pt-1"
                  style={{ color: "rgba(245,158,11,0.52)", fontSize: "9.5px", lineHeight: "1.7",
                    borderTop: "1px solid rgba(245,158,11,0.07)", paddingTop: 10 }}>
                  {activeSegment.methodology}
                </div>
              )}
            </div>
            ...
            <div className="rounded px-4 py-3 flex items-center justify-between"
              style={{ border: "1px solid rgba(245,158,11,0.25)", background: "rgba(245,158,11,0.07)", cursor: "pointer" }}>
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full flex-shrink-0"
                  style={{ background: "#F59E0B", boxShadow: "0 0 5px rgba(245,158,11,0.7)" }} />
              ...
              <span className="font-mono-tactical"
                style={{ color: "rgba(245,158,11,0.55)", fontSize: "10px" }}>
                {activeSegment.label} →
              </span>
            </div>
          </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-1 h-1 rounded-full"
                    style={{
                      background: networkIsLive ? "#F59E0B" : "rgba(155,175,170,0.25)",
                      boxShadow: networkIsLive ? "0 0 4px rgba(245,158,11,0.6)" : undefined,
                    }} />
                  <div className="font-mono-tactical tracking-widest uppercase"
                    style={{ color: "rgba(245,158,11,0.5)", fontSize: "8.5px", letterSpacing: "0.22em" }}>
                    Network Live State
                  </div>
                </div>
                <div className="rounded"
                  style={{ border: "1px solid rgba(245,158,11,0.1)", background: "rgba(0,0,0,0.28)" }}>
                    ...
                    {[
                      ...
                      <div key={m.label} className="flex flex-col gap-0.5 px-4 py-3">
                        ...
                        <span className="font-mono-tactical font-bold"
                          style={{ color: m.live ? "rgba(245,158,11,0.85)" : "rgba(155,175,170,0.38)", fontSize: "22px", lineHeight: 1.1 }}>
                          {m.value}
                        </span>
                        <span className="font-mono-tactical"
                          style={{ color: m.live ? "rgba(245,158,11,0.42)" : "rgba(155,175,170,0.28)", fontSize: "8.5px" }}>
                          {m.sub}
                        </span>
                      </div>
                    ))}
                    ...
                    <div className="flex items-center gap-2 px-4 py-2"
                      ...
                      {hasStaged && (
                        <span className="font-mono-tactical ml-auto"
                          style={{ color: "rgba(245,158,11,0.38)", fontSize: "8.5px" }}>
                  ...
                  <div key={p.id} className="space-y-1.5">
                    <div className="flex items-center gap-2.5">
                      <span className="font-mono-tactical flex-shrink-0"
                        style={{ color: "rgba(245,158,11,0.28)", fontSize: "8px" }}>
                        {p.id}
                      </span>
                      <div className="w-px h-3 flex-shrink-0" style={{ background: "rgba(245,158,11,0.32)" }} />
                      <span className="font-orbitron font-semibold tracking-wider"
                        style={{ color: "rgba(245,158,11,0.72)", fontSize: "9px" }}>
                        {p.label}
                      </span>
                    </div>
      ...
      <div className="px-6 py-4 space-y-2 shrink-0" style={{ borderTop: "1px solid rgba(245,158,11,0.07)" }}>
        <a href="https://www.rsrintel.com" target="_blank" rel="noopener noreferrer"
          ...
          style={{ border: "1px solid rgba(245,158,11,0.18)", background: "rgba(245,158,11,0.05)", textDecoration: "none" }}>
          <div>
            <div className="font-mono-tactical tracking-widest uppercase"
              style={{ color: "rgba(245,158,11,0.45)", fontSize: "7.5px", letterSpacing: "0.14em", marginBottom: 2 }}>
              RSR Intelligence
            </div>
            <div className="font-orbitron font-bold"
              style={{ color: "rgba(245,158,11,0.72)", fontSize: "9.5px" }}>
              rsrintel.com ↗
            </div>
          </div>
          <span style={{ color: "rgba(245,158,11,0.35)", fontSize: "12px" }}>↗</span>
        </a>
