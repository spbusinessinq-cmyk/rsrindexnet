...
const G_FILL_HOV = "rgba(34,197,94,0.15)";
const G_FILL_ACT = "rgba(34,197,94,0.26)";
...
function arcPath(cx: number, cy: number, r1: number, r2: number, startDeg: number, endDeg: number, gapDeg = 2.2): string {
...
function SegmentIcon({ label, x, y, active }: { label: string; x: number; y: number; active: boolean }) {
  ...
  const icons: Record<string, JSX.Element> = {
    OVERVIEW: (
      <g>
        <rect x={-s * 0.8} y={-s * 0.62} width={s * 1.6} height={s * 1.24} rx="1.5" fill="none" stroke={col} strokeWidth="1.2" />
        <line x1={-s * 0.5} y1={-s * 0.2} x2={s * 0.5} y2={-s * 0.2} stroke={col} strokeWidth="1" strokeLinecap="round" />
        <line x1={-s * 0.5} y1={s * 0.08} x2={s * 0.3} y2={s * 0.08} stroke={col} strokeWidth="1" strokeLinecap="round" />
        <line x1={-s * 0.5} y1={s * 0.36} x2={s * 0.1} y2={s * 0.36} stroke={col} strokeWidth="1" strokeLinecap="round" />
      </g>
    ),
    SIGNALS: (
      <g>
        {[s * 0.35, s * 0.62, s * 0.9].map((r, i) => (
          <path
            ...
            strokeOpacity={0.55 + i * 0.2} transform="rotate(180)"
          />
        ))}
    ...
    DATASETS: (
      <g>
        {[-s * 0.42, 0, s * 0.42].map((dy, i) => (
          <rect key={i} x={-s * 0.7} y={dy - s * 0.14} width={s * 1.4} height={s * 0.26} rx="1"
            fill="none" stroke={col} strokeWidth={i === 0 ? "1.3" : "1"} strokeOpacity={i === 0 ? 1 : 0.6} />
        ))}
      </g>
    ),
    INDEX: (
      <g>
        {[-s * 0.48, -s * 0.16, s * 0.16, s * 0.48].map((dy, i) => (
          <g key={i}>
            <circle cx={-s * 0.6} cy={dy} r="1.5" fill={col} fillOpacity={i === 0 ? 1 : 0.55} />
            <line x1={-s * 0.35} y1={dy} x2={s * 0.6} y2={dy} stroke={col} strokeWidth="0.9" strokeLinecap="round" strokeOpacity={0.6} />
          </g>
        ))}
      </g>
    ),
    ...
    ACCESS: (
      <g>
        <rect x={-s * 0.55} y={-s * 0.55} width={s * 1.1} height={s * 1.1} rx="2.5" fill="none" stroke={col} strokeWidth="1.3" />
...
export default function CommandWheel({ segments, onHover, onSegmentClick }: CommandWheelProps) {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center select-none gap-3" style={{ width: SIZE, maxWidth: "100%" }}>
      <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} style={{ overflow: "visible", width: "100%", height: "auto" }}>
        ...
        {[OUTER + 22, OUTER + 38, OUTER + 54].map((r, i) => (
          <circle key={i} cx={cx} cy={cy} r={r} fill="none"
            stroke={G} strokeOpacity={0.04 - i * 0.01} strokeWidth={1}
            strokeDasharray={["5 10", "2 14", "1 20"][i]} />
        ))}

        <circle cx={cx} cy={cy} r={OUTER + 3} fill="none" stroke={G} strokeWidth={1.2} strokeOpacity={0.15} />
        <circle cx={cx} cy={cy} r={INNER - 3} fill="none" stroke={G} strokeWidth={1} strokeOpacity={0.15} />
        ...
        {segments.map((seg, i) => {
          ...
          return (
            <g key={seg.label} data-testid={`segment-${seg.label.toLowerCase()}`} style={{ cursor: "pointer" }}
              ...
              <path d={arcPath(cx, cy, INNER + 4, OUTER - 3, startDeg, endDeg, 2)} fill="transparent" strokeWidth={0} />
              <path d={segPath}
                fill={isClk ? G_FILL_ACT : isHov ? G_FILL_HOV : G_FILL}
                stroke={active ? G : G_FAINT} strokeWidth={active ? 1.4 : 0.8}
                filter={active ? "url(#f-glow)" : undefined}
                style={{ transition: "fill 0.18s ease, stroke 0.18s ease" }} />
              {active && <path d={segPath} fill="none" stroke={G} strokeWidth={2.5} strokeOpacity={0.3} filter="url(#f-glow-lg)" />}
              <path d={tickArc} fill="none" stroke={active ? G : G_FAINT}
                strokeWidth={active ? 2.2 : 1} strokeLinecap="round"
                filter={active ? "url(#f-glow)" : undefined}
                style={{ transition: "stroke 0.18s ease, stroke-width 0.18s ease" }} />
              {active && <path d={tickArc} fill="none" stroke={G} strokeWidth={4} strokeOpacity={0.18} strokeLinecap="round" filter="url(#f-glow-lg)" />}
              ...
              {active && (() => {
                ...
                return <circle cx={arrowPt.x} cy={arrowPt.y} r={3} fill={G} filter="url(#f-glow)" />;
              })()}
            </g>
          );
        })}

        <circle cx={cx} cy={cy} r={INNER - 4} fill="url(#rg-hub)" stroke={G} strokeWidth={1.5} strokeOpacity={0.3} filter="url(#f-glow)" />
        <circle cx={cx} cy={cy} r={INNER - 16} fill="rgba(0,0,0,0.82)" stroke={G} strokeWidth={0.8} strokeOpacity={0.18} strokeDasharray="5 5" />
        <circle cx={cx} cy={cy} r={INNER - 30} fill="rgba(0,0,0,0.6)" stroke={G} strokeWidth={0.5} strokeOpacity={0.1} />
        <circle cx={cx} cy={cy} r={INNER - 42} fill="rgba(0,0,0,0.4)" stroke={G} strokeWidth={0.4} strokeOpacity={0.07} />

        {[0, 60, 120, 180, 240, 300].map((a) => {
          const p1 = polar(cx, cy, INNER - 42, a);
          const p2 = polar(cx, cy, INNER - 18, a);
          return <line key={a} x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y} stroke={G} strokeWidth={0.6} strokeOpacity={0.15} />;
        })}

        <text x={cx} y={cy - 28} textAnchor="middle" dominantBaseline="middle"
          fontSize="7" fontFamily="'Share Tech Mono', monospace" fontWeight="400"
          fill={G} fillOpacity={0.35} letterSpacing="0.35em">DATA</text>
        <text x={cx} y={cy - 6} textAnchor="middle" dominantBaseline="middle"
          fontSize="28" fontFamily="'Orbitron', sans-serif" fontWeight="800"
          fill={G} letterSpacing="0.08em" filter="url(#f-glow-lg)">INDEX</text>
        <text x={cx} y={cy + 18} textAnchor="middle" dominantBaseline="middle"
          fontSize="6.5" fontFamily="'Share Tech Mono', monospace"
          fill={G} fillOpacity={0.4} letterSpacing="0.24em">NETWORK</text>
      </svg>

      <div className="w-full flex items-center justify-center" style={{ height: 48, minHeight: 48 }}>
        {activeSeg ? (
          <div className="flex items-center gap-3 border rounded px-5 py-2.5 transition-all duration-200"
            style={{ borderColor: "rgba(34,197,94,0.28)", background: "rgba(34,197,94,0.05)", boxShadow: "0 0 12px rgba(34,197,94,0.1)", maxWidth: 380 }}>
            <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: G, boxShadow: `0 0 6px ${G}` }} />
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-3">
                <span className="font-orbitron text-xs font-bold tracking-widest" style={{ color: G, textShadow: `0 0 8px rgba(34,197,94,0.5)` }}>
                  {activeSeg.label}
                </span>
                <span className="font-mono-tactical text-xs tracking-widest" style={{ color: "rgba(34,197,94,0.45)" }}>
                  {activeSeg.path}
                </span>
              </div>
              <span className="font-mono-tactical text-xs truncate" style={{ color: "rgba(34,197,94,0.5)" }}>
                {activeSeg.description}
              </span>
            </div>
          </div>
        ) : (
          <div className="font-mono-tactical text-xs tracking-widest" style={{ color: "rgba(34,197,94,0.2)" }}>
            SELECT A SECTOR
          </div>
        )}
      </div>
