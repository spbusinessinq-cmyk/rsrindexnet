...
/* ── Palette ─────────────────────────────────────────────────── */
const G        = "#22c55e";
const G_DIM    = "rgba(34,197,94,0.52)";
...
function WheelIcon({ label, active }: { label: string; active: boolean }) {
  ...
  switch (label) {
    case "OVERVIEW":
      /* Document / structured page icon — tighter */
      return (
...
export default function CommandWheel({ segments, onHover, onSegmentClick }: CommandWheelProps) {
  ...
  /* ── Label/icon geometry — cleanly separated ─────────────────
     Segment midpoint = (INNER + OUTER) / 2 = 168
     Icon centre:  168 - 28 = 140  (icons are ≤10.5px tall → bottom ≈150.5)
     Label centre: 168 + 26 = 194  (gap from icon bottom to label: ~43px)
  ──────────────────────────────────────────────────────────── */
  const SEG_MID  = (INNER + OUTER) / 2;
  const ICON_R   = SEG_MID - 28;   // 140
  const LABEL_R  = SEG_MID + 26;   // 194
    ...
    <div className="relative flex flex-col items-center justify-center select-none gap-1" style={{ width: SIZE, maxWidth: "100%" }}>
      ...
      >
            ...
            >
              ...
              {active && (
                <path d={path} fill="none"
                  stroke={G} strokeWidth={1.4} strokeOpacity={0.35}
                  filter="url(#f-glow)" />
              )}
              {active && (
                <path d={path} fill="none"
                  stroke={G} strokeWidth={4} strokeOpacity={0.08}
        ...
        <g style={{
          ...
          {/* Hub text — three-line hierarchy */}
          <text x={cx} y={cy - 27}
            textAnchor="middle" dominantBaseline="middle"
            fontSize="5.8" fontFamily="'Share Tech Mono', monospace"
            fill={G} fillOpacity={hubActive ? 0.52 : 0.28}
          ...
          <div key={activeSeg.label} className="panel-fade-in flex items-center gap-3 rounded px-5 py-2.5"
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-baseline gap-3 mb-0.5">
                <span className="font-orbitron font-bold tracking-widest" style={{ color: G, fontSize: "9.5px" }}>
                  {activeSeg.label}
                </span>
            ...
            <span className="font-mono-tactical flex-shrink-0 hidden sm:block"
              style={{ color: "rgba(34,197,94,0.35)", fontSize: "9px", letterSpacing: "0.1em" }}>
              ENTER →
            </span>
          </div>
        ) : (
          <div className="flex items-center gap-4">
            <div className="w-8 h-px" style={{ background: "rgba(155,175,168,0.1)" }} />
            <span className="font-mono-tactical tracking-widest"
              style={{ color: "rgba(155,175,168,0.22)", fontSize: "9px", letterSpacing: "0.18em" }}>
